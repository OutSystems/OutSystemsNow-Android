...
public class LoginActivity extends BaseActivity {
    ...
    private void callLoginService(final View v, final String userName, final String password) {
        showLoading(v);
        WebServicesClient.getInstance().loginPlattform(userName, password, new WSRequestHandler() {
            @Override
            public void requestFinish(Object result, boolean error, int statusCode) {
                ...
                if (!error) {
                    ...
                    if (login == null || !login.isSuccess()) {
                        Toast.makeText(getApplicationContext(), login.getErrorMessage(), Toast.LENGTH_SHORT).show();
                    } else {
                        DatabaseHandler database = new DatabaseHandler(getApplicationContext());
